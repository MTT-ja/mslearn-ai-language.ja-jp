...
これで、カスタム固有表現認識プロジェクトを再作成する準備が整いました。 このプロジェクトによって、モデルのビルド、トレーニング、デプロイを行うための作業場所が提供されます。
...
1. **[アクティビティ]** ウィンドウで、このドキュメントがモデルのトレーニング用のデータセットに追加されることに注意してください。
1. **[次のドキュメント]** ボタンを使用して次のドキュメントに移動し、ドキュメント セット全体の適切なエンティティにテキストを割り当て続け、それらをすべてトレーニング データセットに追加します。
...
1. Visual Studio Code の **エクスプローラー** ウィンドウで、**Labfiles/05-custom-entity-recognition** フォルダーを参照し、言語の設定と、それに含まれる  **CSharp** または **Python** を選び、 **custom-entities** フォルダーを展開します。 各フォルダーには、Azure AI Language テキスト分類機能を統合するアプリの言語固有のファイルが含まれています。
...
1. 作成した Azure 言語リソースの**エンドポイント**と**キー**を含むように構成値を更新します (Azure portal の Azure AI Language リソースの **[キーとエンドポイント]** ページで利用可能)。 ファイルには、カスタム エンティティ抽出モデルのプロジェクト名とデプロイ名が既に含まれている必要があります。
1. 構成ファイルを保存します。
...
1. **Main** 関数では、既存のコードが **ads** フォルダー内のすべてのファイルを読み取り、その内容を含む一覧を作成することに注意してください。 C# コードの場合、**TextDocumentInput** オブジェクトの一覧を使用して、ファイル名を ID と言語として含めます。 Python では、テキスト コンテンツの単純な一覧が使用されます。
